...
export class StockController {
  ...
  async create(@Rest() rest, @Body() body, @Res() response, @Me() me: StaffUser) {
    ...
    const progress = async () => {
      ...
      try {
        const items = await Product.findBy({
          id: In(body.products.map((val) => val.id)),
          restaurant_id: rest.id,
        });
        ...
        const locations = await Location.findBy({
          id: In(body.location_ids),
          restaurant_id: rest.id,
        });
        ...
        for (const item of body.products) {
          ...
          for (const location of locations) {
            try {
              ...
              const countVariant = await ProductVariant.count({
                where: { product_id: product.id },
              });
              if (countVariant > 1) {
                const check = await ProductVariant.exists({
                  where: { id: productVariant.id, variant_id: IsNull() },
                });
                if (check) {
    ...
    progress()
      .then(({ success, fails }) => {
        ...
        Socket.getInstance().event(me.id, {
          ...
          type: PubSubEventType.StaffCreateStock,
          payload: {
  ...
  async update(@Rest() rest, @Body() body, @Res() response, @Param() param, @Me() me: StaffUser) {
    ...
    const productStock = await ProductStock.findOneByOrFail({
      restaurant_id: rest.id,
      id: param.stock_id,
    });
